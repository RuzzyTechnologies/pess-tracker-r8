...
export function Preloader({
  ...
  React.useEffect(() => {
    ...
    // Ensure it doesn't hang forever
    ...
    // Let the app hydrate and schedule finish on idle or next frame
    ...
    <AnimatePresence>
      {show ? (
        <motion.div
          ...
          aria-label={label}
        >
          {/* Full background that matches the app aesthetic */}
          <div className="absolute inset-0 bg-gradient-to-b from-sky-50 to-white dark:from-slate-950 dark:to-slate-900" />

          {/* Center content */}
          <div className="relative mx-4 flex max-w-sm flex-col items-center gap-4 rounded-2xl border border-sky-100/70 bg-white/70 p-6 text-center shadow-sm backdrop-blur-md dark:border-slate-800 dark:bg-slate-900/70">
            {/* Logo */}
