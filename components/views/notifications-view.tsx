  ...
  return (
    <div className="mx-auto w-full max-w-5xl p-4 md:p-6 space-y-4">
      ...
      <Card className="backdrop-blur-lg supports-[backdrop-filter]:bg-white/10 border border-white/20 shadow-lg dark:supports-[backdrop-filter]:bg-slate-900/10 dark:border-slate-700/30">
        <CardHeader>
          <CardTitle className="text-slate-800 dark:text-white">Activity</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          {items.length === 0 ? (
            <div className="text-sm text-slate-500 dark:text-slate-300">You're all caught up 🎉</div>
          ) : (
            <ul className="space-y-3">
              {items.map((n) => (
                ...
                >
                  <span className={n.read ? "text-slate-500 dark:text-slate-300" : "text-slate-800 dark:text-white"}>
                    {n.message}
                  </span>
                  <Button
                    variant="outline"
